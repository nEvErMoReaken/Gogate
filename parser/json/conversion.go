...
func ConversionToSnapshot(js string, config *JsonParseConfig, collection *model.SnapshotCollection, comm *common.Config) {
	// 1. 拿到解析函数
	...
	if err != nil {
		common.Log.Errorf("Unmarshal JSON 失败: %v", err)
	}
	// 3. 调用解析函数
	devName, devType, fields, err1 := convertFunc(result)
	if err1 != nil {
		common.Log.Fatalf("解析 JSON 失败: %v, 请检查脚本是否正确", err1)
	}
	// 3. 更新 DeviceSnapshot
